...
passiveOnlyMode  = modelConfig.passiveOnlyMode;
...
assert( lceOpt < 0.6 && lceOpt > 0 ,   'Check Units: fiber length in m!');
...
elseif(useElasticTendon == 1 )
    ...
    if(useFiberDamping == 0)
        ...
        %%
        if passiveOnlyMode
            % disp('[INFO] Passive-only mode: fvN fixed to 1, dlceN = 0');
            fvN   = 1;
        ...
        % fprintf("fvN=%.4e, falN=%.4e, a=%.4e, cosAlpha=%.4e, dlcen=%.4e\n", fvN, falN, a, cosAlpha, dlceN);
    ...
    if passiveOnlyMode
        init.err        = fiso*( ffpN*cosAlpha - ftN );
    else
        init.err        = fiso*( (ffaN + fpeN)*cosAlpha - ftN);
    end
